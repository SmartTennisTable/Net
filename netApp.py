...
while True:
	if len(liste_valeurs) >= 100:
		...
		if moyenne > seuil:
			...
			payload = '{"mode":"sync","messageType":"'+sttiot.SENSOR_NET_MESSAGE_ID+'","messages":[{"id_table":'+float(ID_TABLE)+',"id_match":'+float(ID_MATCH)+',"id_let":'+float(indice_let)+',"timestamp":'+float(timeStamp)+'}]}'
